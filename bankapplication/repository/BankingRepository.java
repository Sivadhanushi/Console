package com.zsgs.bankapplication.repository;
...
import com.zsgs.bankapplication.dto.User;
...
public class BankingRepository {
	...
	private ArrayList<Transaction> accountCredentials = new ArrayList<>();
	...
	public long checkDeposit(int acNo,long balance,Date date)
	{
		...
		for (User uc : UserCredentials) {
			if(uc.getAccountNo()==acNo)
			{
				...
				accountCredentials.add(new Transaction(acNo,balance,date,"Deposit",updatedBalance));		
			}
	...
	public long checkWithdraw(int acNo,long balance,Date date)
	{
		...
		for (User uc : UserCredentials) {
			if(uc.getAccountNo()==acNo)
			{
				...
				updatedBalance=uc.getAccountBalance()-balance;
				uc.setAccountBalance(updatedBalance);
				accountCredentials.add(new Transaction(acNo,balance,date,"Withdraw",updatedBalance));
				}
			}
	...
	public ArrayList<Transaction> viewTransection(int AccountNo)
	{
		ArrayList <Transaction> viewTransection =new ArrayList<>();
		for(Transaction tc: accountCredentials)
		{
